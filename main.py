...
class Lecturer(Mentor):
    ...
    def average_lecturer(self, course):
        ...
        elif not self.grades_lecturer.get(course):
...
def average_course_stud(students_list, course):
    ...
    for student in students_list:
        if not course in student.course_progress:
...
student_1 = Student('Петр', 'Беляев', 'мужской')
student_1.add_course('git')
student_1.course_progress += ['python']

student_2 = Student('Елизавета', 'Андреева', 'женский')
student_2.add_course('python')
student_2.course_progress += ['git']

reviewer_1 = Reviewer('Артём ', 'Никитин')
reviewer_1.course_list += ['python']

reviewer_2 = Reviewer('Егор','Чернов')
reviewer_2.course_list += ['git']

lecturer_1 = Lecturer('Иван', 'Петров')
lecturer_1.course_list += ['python']

lecturer_2 = Lecturer('Максим', 'Новиков')
lecturer_2.course_list += ['git']

reviewer_1.rate_students(student_1,'python', 9)
reviewer_2.rate_students(student_2,'git', 7)
reviewer_1.rate_students(student_1,'python', 5)
reviewer_2.rate_students(student_2,'git', 8)
